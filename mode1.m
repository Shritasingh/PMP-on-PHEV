...
U = opti.variable(1,N+1);   % control trajectory 
twh  = U(1,:);
...
%battery- 
%Pbatt = @(v,twh) wm(v).*tm(twh)./effm;
Pbatt = @(v,twh) wm(v).*tm(twh);
...
I_bat = @(xi,v,twh) (Voc(xi)-sqrt(Voc(xi)^2 - 4*Pbatt(v,twh).*Ro(xi)))./(2*Ro(xi));
p_ech = @(xi,v,twh) Voc(xi).*I_bat(xi,v,twh);

xi_dot= @(xi,v,twh) -n_c*I_bat(xi,v,twh)/Q;
v_dot= @(v,twh) (r_wh/Jv)*(twh - r_wh*(c0 + c1.*v + c2*v.*v));


% Dynamics
f = @(xi,v,twh) [xi_dot(xi,v,twh); v_dot(v,twh)]; % dx/dt = f(x,u)
...
for k=1:N % loop over control intervals
   ...
   k1 = f(xi(:,k), v(:,k), twh(:,k));
   k2 = f(xi(:,k)+dt/2*k1(1), v(:,k)+dt/2*k1(2), twh(:,k));
   k3 = f(xi(:,k)+dt/2*k2(1), v(:,k)+dt/2*k2(2), twh(:,k));
   k4 = f(xi(:,k)+dt*k3(1), v(:,k)+dt*k3(2), twh(:,k));
